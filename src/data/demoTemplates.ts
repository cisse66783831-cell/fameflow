import { Campaign } from '@/types/campaign';

// Demo frame - a simple circular profile frame
const demoFrameBase64 = 'data:image/svg+xml;base64,';

// Demo certificate background
const demoCertBase64 = 'data:image/svg+xml;base64,';

export const demoTemplates: Campaign[] = [
  {
    id: 'demo-photo-1',
    title: 'Profile Frame - Gradient Style',
    description: 'A beautiful gradient frame for profile pictures. Perfect for social media campaigns.',
    type: 'photo',
    frameImage: demoFrameBase64,
    textElements: [],
    hashtags: ['#FrameFlow', '#ProfilePic', '#Viral'],
    views: 1250,
    downloads: 340,
    createdAt: new Date('2024-01-15'),
    isDemo: true,
  },
  {
    id: 'demo-document-1',
    title: 'Certificate of Achievement',
    description: 'Professional certificate template with customizable name and date fields.',
    type: 'document',
    frameImage: '',
    backgroundImage: demoCertBase64,
    textElements: [
      {
        id: 'cert-title',
        label: 'Title',
        value: 'CERTIFICATE OF ACHIEVEMENT',
        x: 400,
        y: 160,
        fontSize: 32,
        fontFamily: 'Georgia',
        color: '#6366f1',
        fontWeight: 'bold',
        isDraggable: false,
      },
      {
        id: 'cert-name',
        label: 'Recipient Name',
        value: 'John Doe',
        x: 400,
        y: 280,
        fontSize: 42,
        fontFamily: 'Georgia',
        color: '#1e1b4b',
        fontWeight: 'bold',
        isDraggable: true,
      },
      {
        id: 'cert-desc',
        label: 'Description',
        value: 'For outstanding contribution and excellence',
        x: 400,
        y: 350,
        fontSize: 18,
        fontFamily: 'Arial',
        color: '#64748b',
        fontWeight: 'normal',
        isDraggable: true,
      },
      {
        id: 'cert-date',
        label: 'Date',
        value: 'December 15, 2024',
        x: 400,
        y: 480,
        fontSize: 16,
        fontFamily: 'Arial',
        color: '#64748b',
        fontWeight: 'normal',
        isDraggable: true,
      },
    ],
    hashtags: ['#Certificate', '#Achievement', '#Award'],
    views: 890,
    downloads: 215,
    createdAt: new Date('2024-02-10'),
    isDemo: true,
  },
];
